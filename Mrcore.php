...
class Mrcore extends Parser
{
    ...
    private function preParseFiles($params)
    {
        ...
        if (preg_match('"file=(.*);"Ui', $params, $matches)) {
            ...
            if (preg_match('"title=(.*);"Ui', $params, $matches)) {
                $title = $matches[1];
            }
            if (preg_match('"view=(.*);"Ui', $params, $matches)) {
                $view = strtolower($matches[1]);
            }
            ...
            $path = '';
            if (preg_match('"path=(.*);"Ui', $params, $matches)) {
                $path = $matches[1];
            }
            $path = $standardizePath($path);
